...
const FollowList = (props) => {
    ...
    const renderItemComponent = (data) =>
        <View style={styles.userList}>
            <TouchableOpacity style={styles.header} onPress={() => { toUser(data.item.followed, data.item.users.roles) }}>
                {
                    data.item.users.avatar !== null ? (
                        <Image source={{ uri: API_URL() + data.item.users.avatar }} style={styles.personImage} />
                    ) : (
                        <Image source={image.profile_image} resizeMode="cover" style={styles.personImage} />
                    )
                }
                <Text style={{ fontSize: 15, marginLeft: 5 }}>{data.item.users.username}</Text>
            </TouchableOpacity>
        </View>
